...
class MyHyperModel(kt.HyperModel):
    def build(self, hp):
        ...
        x= layers.LSTM(hp.Choice('LSTM1', [4, 8, 16, 32], default=16))(input_layer)
        x= layers.Dropout(hp.Float('dropout1', min_value=0, max_value=0.8, step=0.1, default=0.8))(x)


        y= layers.Permute((2, 1))(input_layer)
        y= layers.Conv1D(hp.Choice('conv1', [32, 64, 128, 256], default=128), hp.Choice('ksize1', [3, 5, 8], default=8), 1, padding='same', kernel_initializer='he_uniform')(y)
        y= layers.BatchNormalization()(y)
        y= layers.Activation('relu')(y)
        y= layers.Conv1D(hp.Choice('conv2', [32, 64, 128, 256], default=256), hp.Choice('ksize2', [3, 5, 8], default=5), 1, padding='same', kernel_initializer='he_uniform')(y)
        y= layers.BatchNormalization()(y)
        y= layers.Activation('relu')(y)
        y= layers.Conv1D(hp.Choice('conv3', [32, 64, 128, 256], default=128), hp.Choice('ksize3', [3, 5, 8], default=3), 1, padding='same', kernel_initializer='he_uniform')(y)
        y= layers.BatchNormalization()(y)
        y= layers.Activation('relu')(y)
...
def LSTMFCN_classifier(X, Y, x_test, y_test, params):
