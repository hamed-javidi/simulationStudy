...
class MyHyperModel(kt.HyperModel):
    def build(self, hp):
        ...
        model.add(layers.Conv1D(hp.Choice('conv1', [32, 64, 128, 256], default=32), hp.Choice('ksize1', [3, 5, 8], default=5), 1, padding='same'))
        model.add(layers.Activation('relu'))
        model.add(layers.MaxPooling1D(2, padding='same'))
        model.add(layers.Conv1D(hp.Choice('conv2', [32, 64, 128, 256], default=64), hp.Choice('ksize2', [3, 5, 8], default=3), 1, padding='same'))
        model.add(layers.Activation('relu'))
        model.add(layers.LSTM(hp.Choice('LSTM1', [4, 16, 64, 128], default=64), dropout= hp.Float('dropout1', min_value=0, max_value=0.8, step=0.1, default=0.1),
                              recurrent_dropout=hp.Float('re_dropout1', min_value=0, max_value=0.8, step=0.1, default=0.5)))
...
def CNN_RNN_classifier(X, Y, x_test, y_test, params):
