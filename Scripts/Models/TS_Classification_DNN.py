...
def run_main_code(selected_classifier, seed, test_frac, verbose=False, k_fold=3):
    ...
    print(root_path)
    print(patient_cohorts)
    patient_cohorts.remove('hyperparameter_cohorts')

    # To recover existing results
    if recovery_mode:
        path = save_path + selected_classifier + '_' + vital_type + '_' + cohort_name + '_metrics.csv'
        recovered_results = pd.read_csv(path)

    for patient_cohort in patient_cohorts:
        sub_cohorts = directory_contents(root_path + patient_cohort + '/')

        for sub_cohort in sub_cohorts:
            fnames = directory_contents(root_path + patient_cohort + '/' + sub_cohort + '/', 1)
            file_path = root_path + patient_cohort + '/' + sub_cohort

            for fname in fnames:
                print(f'{selected_classifier} is running\n'
                      f'Path: {file_path}\n'
                      f'File name: {fname}')
                ...
                for imb_ratio in [0.25, 0.5, 0.75, 1]:
                    ...
                    # if not (missingness == '0' and irregularity == 'none' and imb_ratio == 0.25):
                    #     continue
                    ...
                    params = {
                        ...
                        "folds": k_fold,
                        "tune_project_name": # save_name
                            vital_type + '_' + cohort_name + '_' + mean + '_' + std + '_' + irregularity + '_' + str(imb_ratio)
                            # + '_' + patient_cohort + '_' + irregularity + '_' + missingness + '_' + str(imb_ratio)
                    }
                ...
                if len(df_metrics) > 0:
                    pd.concat(df_metrics, axis=0, sort=False).to_csv(save_path + selected_classifier + '_' + vital_type + '_' + cohort_name + '_metrics.csv',
                                                                 index=False)
    ...
    if len(df_metrics) > 0:
        ...
        df_metrics.to_csv(save_path + selected_classifier + '_' + vital_type + '_' + cohort_name + '_metrics.csv', index=False)
...
if __name__ == "__main__":
    ...
    total_files = 528
    verbose = False
    recovery_mode = 0  # To reuse results from the last unfinished run
