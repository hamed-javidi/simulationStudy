...
def RNN_classifier(save_path, filename, x_train, y_train, x_val, y_val, nb_classes, run, verbose=False, min_exp_val_loss=0.005):
        np.random.seed()
        batch_size = 50
        nb_epochs = 500
        # FCNN Model
        input_layer = Input(shape = x_train.shape[1:])
        x = layers.LSTM(16, dropout=0.1, recurrent_dropout=0.2, return_sequences=True)(input_layer)
        x = layers.LSTM(32, dropout=0.1, recurrent_dropout=0.3, return_sequences=True)(x)
