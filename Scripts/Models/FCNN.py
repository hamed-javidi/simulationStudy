...
class MyHyperModel(kt.HyperModel):
    def build(self, hp):
        ...
        model.add(layers.Conv1D(hp.Choice('conv1', [32, 64, 128, 256], default=128), hp.Choice('ksize1', [3, 5, 8], default=8), 1, padding='same'))
        model.add(layers.BatchNormalization())
        model.add(layers.Activation('relu'))
        model.add(layers.Conv1D(hp.Choice('conv2', [32, 64, 128, 256], default=256), hp.Choice('ksize2', [3, 5, 8], default=5), 1, padding='same'))
        model.add(layers.BatchNormalization())
        model.add(layers.Activation('relu'))
        model.add(layers.Conv1D(hp.Choice('conv3', [32, 64, 128, 256], default=128), hp.Choice('ksize3', [3, 5, 8], default=3), 1, padding='same'))
        model.add(layers.BatchNormalization())
        model.add(layers.Activation('relu'))
...
def FCNN_classifier(X, Y, x_test, y_test, params):
    """
    np.random.seed()
    batch_size = 50
    nb_epochs = 500
    ...
    x = layers.Conv1D(128, 8, 1, padding='same')(input_layer)
    x = layers.BatchNormalization()(x)
    x = layers.Activation('relu')(x)
    x = layers.Conv1D(256, 5, 1, padding='same')(x)
    x = layers.BatchNormalization()(x)
    x = layers.Activation('relu')(x)
    ...
    file_path = save_path + 'models/' + filename + '_FCNN_best_model_run_' + str(run) + '.hdf5'
    ...
    hist_df.to_csv(save_path + 'models/' + filename + '_FCNN_history_run_' + str(run) + '.csv', index=False)
